...
func Build(form map[string]any) (Request, error) {
	...
	req := Request{
		...
		AdditionalNote: str(form, "additionalNote"),
	}
	if req.CusdecNo == "" {
	...
	if strings.EqualFold(str(form, "cargoType"), LCL) {
		return Request{}, &buildError{
...
func buildContainers(form map[string]any) ([]Container, error) {
	...
	for i, raw := range rows {
		...
		containers = append(containers, Container{
			ContainerNo:   str(row, "containerNo"),
			ContainerType: str(row, "containerType"),
			ContainerSize: str(row, "containerSize"),
			ServiceID:     serviceID,
...
func str(m map[string]any, key string) string {
	...
	case float64:
		// A form number where the CMS wants a string — container_size is the one
		// that happens with, since the declaration offers it as a choice.
