...
| `expectAbsent` | List of dot-notation paths that must **not** exist — how a flow proves a response withheld something. |

A step asserting only `expectStatus` never parses the body. The three fields
above are what make it worth parsing, and `expectAbsent` is the one a status
code cannot substitute for: a 200 tells you the request was allowed, not which
parts of the answer the caller was entitled to.
