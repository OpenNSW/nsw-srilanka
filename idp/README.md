...
> **Pinned version: `1.0.0-beta`** (`compose.yml`). Note the OCI tag has no `v`
...
| --- | --- | --- | --- |
| TraderApp users | NSW_API | `VITE_IDP_RESOURCE` (SPA `extraQueryParams`) | `https://api.nsw-srilanka.local` |
| OGA portal users | AGENCY_API | `VITE_IDP_RESOURCE` in OpenNSW/nsw-agency | `https://api.nsw-agency.local` |
| `*_TO_NSW` M2M | NSW_API | `NSW_TOKEN_URL` query in OpenNSW/nsw-agency | `https://api.nsw-srilanka.local` |
| `NSW_TO_*` M2M | AGENCY_API | `token_url` query in `configs/services*.json` | `https://api.nsw-agency.local` |
...
**M2M clients** append it to the token URL as a query parameter — e.g.
`https://thunderid:8090/oauth2/token?resource=https://api.nsw-agency.local`. This works
because the token endpoint calls `r.ParseForm()`, which merges the URL query into the
form, and needs no client-library support: neither `github.com/OpenNSW/core/remote` nor
`golang.org/x/oauth2/clientcredentials` exposes a field for `resource`.
