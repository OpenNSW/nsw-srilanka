...
# Clear a server configuration section (PUT with an empty value).
#
# There is no DELETE for server config. A PUT replaces only the *writable* layer —
# the readOnly layer loaded from declarative files is untouched — so clearing removes
# exactly what the seed applied and nothing else.
reset_server_config() {
