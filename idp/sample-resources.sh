...
JSON
    ...
    # Duplicate application: APP-1020 (name) / APP-1021 (clientId). NOT APP-1022,
    # which is "Invalid JWKS URI scheme" and would be swallowed as "already exists".
...
create_resource_server() {
    ...
    # `type` is immutable after creation; changing it later means recreating the
    # resource server and orphaning its permission grants.
...
bootstrap_registry() {
    ...
    # Five shipped flows share the handle `default-flow`; the flowType filter selects one.
...
process_resource_servers() {
    ...
    while IFS= read -r obj; do
        [[ -z "$obj" ]] && continue
        name="$(jq -r '.name' <<< "$obj")"
        # The identifier is both the key other resource files reference via
        # "resourceServer" and the token `aud`, so it must be an absolute URI.
...
# Server-level runtime configuration (PUT /server-config/<name>). Seeded here rather
# than in the bootstrap bundle so a bad value fails only the seed, not the image's own
# default resources. `allowedOriginsEnv` names an indexed env list (NAME_0, NAME_1, ...)
