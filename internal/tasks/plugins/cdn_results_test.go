...
func branch(cdnNumber, edgeID string, accepted bool) map[string]any {
	return map[string]any{
		"userform": map[string]any{
			"noteDetails": map[string]any{"cdnNumber": cdnNumber},
...
// The number Customs registered wins over the trader's own note reference.
func TestCDNResultsCollector_PrefersRegisteredNumber(t *testing.T) {
	b := branch("CDN-LOCAL", "edge-1", true)
	b["userform"].(map[string]any)["registeredCdnNumber"] = "CBEX1/2026/C/28237"

	ctx := splitCtx(map[string]any{"cdn_results": []any{b}})
...
func TestCDNResultsCollector_SkipsRejectedForFirstNote(t *testing.T) {
	...
	assert.Equal(t, "CDN-2", userform["noteDetails"].(map[string]any)["cdnNumber"])
}
