...
//   - internal/tasks/extensions/stepauthz — Layer 2 for writes. A PRE_RESUME task
//     extension deciding whether the caller may run a command on a task.
//   - internal/tasks/readauthz — Layer 2 for reads. Decides whether the caller
//     may read a task, and which role they read it as.
//
// This package exists because both Layer-2 evaluators, and Layer 1, need the same
// principal types and the same eligibility rule. Only one of the three is a core
// task extension, so the shared half cannot live under extensions/.
package taskauthz
...
// WithInput returns a context carrying in for an evaluator to read.
