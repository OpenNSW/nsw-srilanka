// Package readauthz decides whether a caller may read a task, and resolves the
// per-role claims that shape what they see inside it.
//
...
// that against the catalog and the task's render config. It touches no domain
// service and reads no configuration file.
//
// Eligibility — holding a role and owning the task's consignment in it — is
// decided by taskauthz, shared with the write path. What this package adds is
// that a read selects *content* rather than deciding one yes/no question, so it
// cannot simply take any match.
//
...
// "waiting for your CHA"). So when a render config declares read.roles, that list
// is a precedence order: the caller acts as the first role in it they are
...
// Resolve decides whether the caller may read the task rooted at rootWorkflowID,
// which is the id of the consignment the task belongs to, and if so returns the
// claims that shape their view of it. It returns ErrDenied when they may not; any
// other error is a real failure, not a denial, and must not be answered as one.
//
// The returned map always carries one entry per catalog role, denials included as
// false: uiprojector treats a claim a blueprint references but the caller never
// populated as a caller bug and fails the whole render, so a partially populated
// map would turn a policy decision into a 500.
//
...
func Resolve(ctx context.Context, cat taskauthz.Catalog, in taskauthz.Input, renderConfig json.RawMessage, rootWorkflowID string) (map[string]bool, error) {
	allowed, declared, err := allowedRoles(ctx, cat, renderConfig)
	if err != nil {
		return nil, err
	}
	...
	claims := make(map[string]bool, len(cat.Roles))
	for _, name := range cat.RoleNames() {
...
// startup failure.
func allowedRoles(ctx context.Context, cat taskauthz.Catalog, renderConfig json.RawMessage) ([]string, bool, error) {
	var policy Policy
	...
	if policy.Read == nil || len(policy.Read.Roles) == 0 {
		return cat.RoleNames(), false, nil
	}
