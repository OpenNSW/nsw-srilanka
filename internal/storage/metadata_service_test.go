...
func TestObjectMetadataService_ValidateAccess(t *testing.T) {
	...
	t.Run("upsert enrichment updates task_id and preserves uploaded_by and inverse", func(t *testing.T) {
		testValidateAccessUpsertEnrichment(t, db, svc, ctx)
	})
}

func testValidateAccessUpsertEnrichment(t *testing.T, db *gorm.DB, svc *ObjectMetadataService, ctx context.Context) {
	enrichKey := "00000000-0000-0000-0000-000000000002.pdf"
	if err := svc.RecordMetadata(ctx, enrichKey, "initial-task", "c-2", "company-trader-2", "initial-user"); err != nil {
		t.Fatalf("initial RecordMetadata failed: %v", err)
	}

	// Update with non-empty taskID and empty uploadedBy -> task_id updates, uploaded_by remains unchanged
	if err := svc.RecordMetadata(ctx, enrichKey, "updated-task", "", "", ""); err != nil {
		t.Fatalf("enrichment RecordMetadata failed: %v", err)
	}

	var rec ObjectMetadata
	if err := db.First(&rec, "key = ?", enrichKey).Error; err != nil {
		t.Fatalf("failed to query record: %v", err)
	}
	if rec.TaskID != "updated-task" {
		t.Errorf("expected task_id to update to 'updated-task', got %q", rec.TaskID)
	}
	if rec.UploadedBy != "initial-user" {
		t.Errorf("expected uploaded_by to remain 'initial-user', got %q", rec.UploadedBy)
	}

	// Inverse update: non-empty uploadedBy and empty taskID -> uploaded_by updates, task_id is preserved
	if err := svc.RecordMetadata(ctx, enrichKey, "", "", "", "updated-user"); err != nil {
		t.Fatalf("inverse enrichment RecordMetadata failed: %v", err)
	}

	if err := db.First(&rec, "key = ?", enrichKey).Error; err != nil {
		t.Fatalf("failed to query record after inverse update: %v", err)
	}
	if rec.UploadedBy != "updated-user" {
		t.Errorf("expected uploaded_by to update to 'updated-user', got %q", rec.UploadedBy)
	}
	if rec.TaskID != "updated-task" {
		t.Errorf("expected task_id to remain preserved as 'updated-task', got %q", rec.TaskID)
	}
}
